...
from recommender_systems.metrics import mean_square_error, mean_absolute_error, root_mean_square_error
...
class EmbeddingsARS(AbstractRecommenderSystem, ABC):
    ...
    def _fit(self, epochs: int, verbose: bool) -> None:
        ...
        list_of_epochs = tqdm(range(epochs)) if verbose else range(epochs)
    ...
    def fit(self, data: sparse.coo_matrix, epochs: int, debug_name: tp.Optional[str] = None, verbose=True) -> 'EmbeddingsARS':
        """
        Method for training a model
    ...
    def refit(self, data: sparse.coo_matrix, epochs: int = 100, debug_name: tp.Optional[str] = None, verbose=True) -> 'EmbeddingsARS':
        """

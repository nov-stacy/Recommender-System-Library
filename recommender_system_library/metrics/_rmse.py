...
def _one_root_mean_square_error(true_ratings: np.ndarray, predicted_ratings: np.ndarray) -> float:
    """
    Method to calculate RMSE for one user
    ...
    return np.sqrt(np.sum((predicted_ratings - true_ratings) ** 2) / predicted_ratings.shape[0])
...
def root_mean_square_error(true_ratings: tp.List[np.ndarray], predicted_ratings: tp.List[np.ndarray]) -> float:
    ...
    def one_root_mean_square_error(index) -> float:
        return _one_root_mean_square_error(true_ratings[index], predicted_ratings[index])

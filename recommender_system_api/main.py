...
def change_recommender_system(system_id: int) -> None:
    user_id = backend.check_user_token(flask.request.headers)
    backend.change_recommender_system(user_id, system_id, flask.request.json)
...
def train_recommender_system(system_id: int) -> None:
    user_id = backend.check_user_token(flask.request.headers)
    backend.train_recommender_system(user_id, system_id, flask.request.json)
...
def clear_recommender_system(system_id: int) -> None:
    user_id = backend.check_user_token(flask.request.headers)
    backend.clear_recommender_system(user_id, system_id)
...
def get_predicted_items(system_id: int) -> tp.Dict[str, tp.Any]:
    user_id = backend.check_user_token(flask.request.headers)
    return backend.get_list_of_items_from_recommender_system(user_id, system_id, flask.request.json)
...
def get_metric_for_system(metric_name: str, system_id: int) -> tp.Dict[str, tp.Any]:
    user_id = backend.check_user_token(flask.request.headers)
    return backend.get_metric_for_recommender_system(user_id, metric_name, system_id, flask.request.json)
